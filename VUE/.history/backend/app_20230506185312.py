...
def nodeinfo():
    ...
    values=[]
    if feature=='os_load5':
        values=db.session.query(os_load5).filter(os_load5.cluster_name==c_name and os_load5.node_name==n_name).all()
    elif feature=='process_cpu_percent':
        values=db.session.query(process).filter(process.cluster_name==c_name and process.node_name==n_name).all()
    elif feature=='index_time_seconds_total':
        values=db.session.query(index).filter(index.cluster_name==c_name and index.node_name==n_name).all()
    elif feature=='search_query_time_seconds':
        values=db.session.query(query).filter(query.cluster_name==c_name and query.node_name==n_name).all()
    elif feature=='transport_rx_size_bytes_total':
        values=db.session.query(rx).filter(rx.cluster_name==c_name and rx.node_name==n_name).all()
    elif feature=='transport_tx_size_bytes_total':
        values=db.session.query(tx).filter(tx.cluster_name==c_name and tx.node_name==n_name).all()
    #return jsonify('hello')
    return jsonify(list(map(serialzed,values)))
